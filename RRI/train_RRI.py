"""
miRNA-mRNA Interaction Prediction Based on Precomputed Embeddings
...
def parse_config():
    parser = argparse.ArgumentParser(description="miRNA-mRNA Interaction Prediction Based on Precomputed Embeddings")
    
    # Data related parameters
    parser.add_argument("--data_dir", type=str, required=True, 
                      help="Data directory, should contain files from train_fold_0.csv to train_fold_9.csv")
    parser.add_argument("--prot_embed_path", type=str, required=True,
                      help="Precomputed miRNA embedding pickle file path")
    parser.add_argument("--prot_mask_path", type=str, required=True,
                      help="Precomputed miRNA attention mask pickle file path")
    ...
    parser.add_argument("--prot_id_col", type=str, default="miRNA_ID",
                      help="miRNA ID column name in CSV")
    ...
    parser.add_argument("--prot_dim", type=int, default=1280,
                      help="miRNA embedding dimension")
    ...
    parser.add_argument("--reduction_dim", type=int, default=128,
                      help="Dimension reduction target dimension for miRNA and mRNA embeddings when fusion is None")
...
class EmbeddingDataset(Dataset):
    ...
    def __init__(self, df, prot_embeddings, drug_embeddings, prot_id_col, drug_id_col, label_col):
        """
        Args:
            df: DataFrame containing miRNA ID, mRNA ID, and labels
            prot_embeddings: Dictionary mapping miRNA ID to embeddings
            drug_embeddings: Dictionary mapping mRNA ID to embeddings
            prot_id_col: miRNA ID column name in DataFrame
    ...
    def __getitem__(self, idx):
        try:
            ...
            if prot_data is None:
                raise KeyError(f"miRNA ID {prot_id} does not exist in embedding dictionary")
...
def load_data_and_embeddings(args, fold):
    ...
    print(f"miRNA embeddings loaded successfully, time taken: {prot_embed_time:.2f} seconds")
    ...
    print(f"miRNA masks loaded successfully, time taken: {prot_mask_time:.2f} seconds")
    ...
    # Merge masks into embeddings
    print("\nMerging embeddings and masks...")
    merge_start_time = time.time()
    
    print("Processing miRNA data...")
    prot_ids = list(prot_embeddings.keys())
    for prot_id in tqdm(prot_ids, desc="Merging miRNA embeddings and masks"):
        if prot_id in prot_masks:
            ...
            print(f"Warning: miRNA ID {prot_id} does not exist in masks")
    ...
    if missing_prots:
        print(f"Warning: {len(missing_prots)} miRNA IDs do not exist in embeddings")
        if len(missing_prots) < 10:
            print(f"Missing miRNA IDs: {missing_prots}")
        else:
            print(f"Some missing miRNA IDs: {missing_prots[:10]}...")
    ...
    print(f"miRNA embedding loading time: {prot_embed_time:.2f} seconds ({prot_embed_time/total_time*100:.1f}%)")
    print(f"miRNA mask loading time: {prot_mask_time:.2f} seconds ({prot_mask_time/total_time*100:.1f}%)")
    ...
    print(f"miRNA embeddings: {len(prot_embeddings)}, mRNA embeddings: {len(drug_embeddings)}")
    ...
    for epoch in range(args.num_epochs):
        ...
        with torch.no_grad():
            ...
            try:
                ...
                # Handle case where only one class exists in validation set
...
def evaluate(model, test_loader, device):
    ...
    # Convert prediction probabilities to binary classification labels
...
def select_best_model(metrics_history):
    # Use a weighted average of multiple metrics
    combined_scores = []
...
def k_fold_cross_validation(args):
    ...
    for fold in range(args.n_folds):
        ...
        try:
            ...
            if args.setting == 'miRNA_coldstart':
            ...
            elif args.setting == 'mRNA_coldstart':
                # mRNA cold start: Similar logic to RNA cold start
...
def main():
    ...
    # Automatically detect setting type
    data_dir = args.data_dir.lower()
    if  'mrna_coldstart' in data_dir: 
        args.setting = 'mRNA_coldstart'
    elif 'mirna_coldstart' in data_dir:  
        args.setting = 'miRNA_coldstart'
    else:
