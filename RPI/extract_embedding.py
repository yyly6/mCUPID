...
def parse_config():
    parser = argparse.ArgumentParser(description="Compute embeddings for proteins and mRNA")
    
    # Data related parameters
    parser.add_argument("--csv_path", type=str, required=True,
                      help="Input CSV file path containing protein and mRNA sequences")
    ...
    parser.add_argument("--prot_encoder_path", type=str, required=True,
                      help="Protein encoder model path")
    ...
    parser.add_argument("--drug_max_length", type=int, default=1024,
...
def load_models(prot_encoder_path, drug_encoder_path, device):
    ...
    prot_model = EsmForMaskedLM.from_pretrained(prot_encoder_path)
...
class PreEncoded:
    ...
    def encoding(self, prot_input_ids, prot_attention_mask, drug_input_ids, drug_attention_mask):
        with torch.no_grad():
            # Compute protein sequence embeddings
...
def process_csv_and_compute_embeddings(csv_path, output_dir, prot_encoder_path, drug_encoder_path, 
    ...
    required_columns = ['Protein_ID', 'UTR_ID', 'Protein_Sequence', 'UTR_Sequence']
    ...
    print(f"Extracted {len(unique_prots)} unique protein sequences")
    ...
    print("Processing protein sequences in batches...")
    ...
    # Process mRNA sequences in batches
